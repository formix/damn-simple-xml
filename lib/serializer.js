...
function _serialize(root, options, fieldPath, level, buffer, emitter, callback) {
    ...
    // simple autoclosing tag, decrease the xml depth level and exits the 
    // function. 
    ...
    async.series([function(done) {
        ...
        bufferize(buffer, "<" + root.name, level, emitter);
        attrset = addAttributes(root, options, fieldPath, level, buffer, emitter, callback);
        tagtype = closeOpeningTag(root, options, fieldPath, level, buffer, emitter, callback);
        done();
    }, function(done) {
        ...
        // Terminate the current tag and decrease the xml depth level.
        level--;
        if (tagtype !== "autoclose") {
            bufferize(buffer, "</" + root.name + ">", level, emitter);
        }
...
function addAttributes(root, options, fieldPath, level, buffer, emitter, callback) {
    ...
    for (var i = 0; i < attributes.length; i++) {
        ...
        if (value !== undefined) {
            ...
            bufferize(buffer, " " + name, level, emitter);
            if (value != null) {
                var attrValue = "";
                ...
                bufferize(buffer, "=\"" + attrValue + "\"", level, emitter);
            }
        }
    }
...
function closeOpeningTag(root, options, fieldPath, level, buffer, emitter, callback) {
    ...
    if ((root.data === null) || (root.data === undefined)) {
        ...
        if (attributes.length < fieldCount) {
            ...
            bufferize(buffer, " />", level - 1, emitter);
            return "autoclose";
        }
    }
    return "opening";
...
function createObject(parentPath, nodeName, arrays, newOptions) {
    var obj = {};
    var fullName = parentPath + "." + nodeName;
    var anyDepthName = "*." + nodeName;
    if (arrays[fullName] || arrays[anyDepthName]) {
        obj = [];
        if (!arrays[anyDepthName]) {
