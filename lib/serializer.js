...
function addCDataMeta(newMeta, stack) {
    var substack = stack.slice(0, stack.length - 1);
    var fieldPath = createNodePath(substack);
    if (!newMeta.cdatas[fieldPath]) {
        newMeta.cdatas[fieldPath] = [];
    }
    newMeta.cdatas[fieldPath].push(stack.peek().name);
}

function cleanData(meta) {
    cleanArray(meta, "attributes");
    cleanArray(meta, "cdatas");
}

function cleanArray(meta, arrayName) {
    var key, value, i;
    for (key in meta[arrayName]) {
        var items = meta[arrayName][key];
        var arrayItems = {};
...
function serialize(root, callback) {
    ...
    var localMeta = merge.recursive(true, this.meta, root.meta);
    var emitter = this;
    process.nextTick(function() {
        var buffer = { data: "" };
        _serialize(root, localMeta, root.name, 1, buffer, emitter, callback); 
    });
}
...
function _serialize(root, meta, fieldPath, level, buffer, emitter, callback) {
    ...
    async.series([function(done) {
        ...
        tagtype = closeOpeningTag(root, meta, fieldPath, level, buffer, callback);
        done();
    }, function(done) {
        // creates the inner xml data.
        if (tagtype !== "autoclose") {
            createTagContent(root, meta, fieldPath, level, attrset, buffer, emitter, callback);
        }
        done();
    }, function(done) {
...
function createTagContent(root, meta, fieldPath, level, attrset, buffer, emitter, callback) {
